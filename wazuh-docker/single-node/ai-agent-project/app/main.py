...
async def process_single_alert(alert: Dict[str, Any]) -> None:
    ...
    start_time = datetime.now()
        ...
        # Prometheus 監控 - 計時 Embedding API 調用
        ...
        api_call_duration.labels(stage='embedding').observe(embedding_duration)
        
        # 估算 token 使用量 (基於文本長度的近似值)
        alert_text = f"{rule.get('description', '')} {json.dumps(alert_source)}"
        estimated_tokens = len(alert_text.split()) * 1.3  # 粗略估算
        embedding_input_tokens_total.inc(estimated_tokens)
        
        logger.info(f"   ✅ 向量化完成 (維度: {len(alert_vector)}, 模型: Gemini text-embedding-004, 耗時: {embedding_duration:.2f}s)")
        ...
        api_call_duration.labels(stage='llm_analysis').observe(llm_duration)
        
        # 估算 LLM token 使用量
        input_text = alert_summary + " ".join(str(v) for v in formatted_context.values())
        input_tokens_estimate = len(input_text.split()) * 1.3
        output_tokens_estimate = len(analysis_result.split()) * 1.3
        llm_input_tokens_total.inc(input_tokens_estimate)
        llm_output_tokens_total.inc(output_tokens_estimate)
        ...
        # Prometheus 監控 - 記錄成功處理的警報
        alerts_processed_total.inc()
        
        # 記錄總處理時間
        total_duration = (datetime.now() - start_time).total_seconds()
        alert_processing_duration.observe(total_duration)
        
        logger.info(f"🎯 警報 {alert_id} 處理完成 - 總耗時: {total_duration:.2f}s")
        
    except Exception as e:
        # Prometheus 監控 - 記錄處理錯誤
