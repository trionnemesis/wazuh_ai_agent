...
### 參與方式
1. **問題回報**: 使用 GitHub Issues 回報問題或功能需求
2. **代碼貢獻**: 提交 Pull Request，包含測試和文檔
3. **技術討論**: 參與 Discussions 分享使用經驗
...
# 代碼質量檢查
docker exec -it ai-agent python -m black --check app/
docker exec -it ai-agent python -m flake8 app/

# 單元測試執行
docker exec -it ai-agent python -m pytest tests/ -v
```
...
### 開源授權
本專案採用 **GPL v2** 授權，詳見 [LICENSE](wazuh-docker/LICENSE) 文件。

### 技術致謝
- **Wazuh Project**: 開源SIEM平台基礎
- **Google AI Platform**: Gemini Embedding技術
- **Anthropic**: Claude語言模型
- **OpenSearch**: 高性能向量搜索引擎
- **LangChain**: LLM應用開發框架
...
**Wazuh AgenticRAG** - 引領安全運營進入AI時代，讓機器智能為您的網路安全保駕護航。

---

> **版本資訊**: 基於 Wazuh v4.7.4 | **最後更新**: 2024年1月 | **技術等級**: 生產就緒
